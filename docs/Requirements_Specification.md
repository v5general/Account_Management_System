...
| v2.1 | 2026-02-23 | V5General | 新增：支出审核流程、费用分类类型字段（收入/支出）；修改：软删除字段改为is_deleted（0-未删除，1-已删除）；调整：将第7章功能移到第4章之后 |
